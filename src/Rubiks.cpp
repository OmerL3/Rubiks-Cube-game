...
glm::mat4 Rubiks::translate_by(int x, int y, int z)
{
    glm::vec3 trans_safe = cube_to_location.size() % 2 == 1 ? glm::vec3(0.0f) : glm::vec3(0.5f);
    return glm::translate(glm::mat4(1.0f), glm::vec3(static_cast<float>(x), static_cast<float>(y), static_cast<float>(z)) + trans_safe);    
}

Rubiks::Rubiks(int num_of_cubes, Shader* shader, Texture* tex, VertexArray* va) : 
...
void Rubiks::set_char_to_wall(int num_of_cubes)
{
    for(int d = 0; d<3; d++)
    {
...
std::vector<int> Rubiks::get_cube_indices(int wall)
{
...
bool Rubiks::can_rotate(int wall)
{
    int num_of_cubes = cube_to_location.size();
...
glm::vec3 Rubiks::get_rot_axis(int wall)
{
    int num_of_cubes = cube_to_location.size();
...
void Rubiks::apply_rotation(int wall)
{
    if(is_rotating != -1) 
    {
...
void Rubiks::update_cube_locations(int wall)
{
    int num_of_cubes = cube_to_location.size();
...
void Rubiks::rotate_wall(int wall)
{
...
void Rubiks::set_rotate_degree(float mult)
{ 
    rotate_degree = glm::clamp((int) (rotate_degree * mult), 1, 4); 
}

void Rubiks::rand_mix()
{
...
void Rubiks::model_rotate(float angle, glm::vec3 axis)
{
    rubiks_rot = glm::rotate(glm::mat4(1.0f), angle, axis) * rubiks_rot;
}

void Rubiks::change_center_of_rotation(char dir)
{
    int num_of_cubes = cube_to_location.size();
...
void Rubiks::set_clockwise()
{
    if(is_rotating == -1) clockwise = !clockwise; 
}

void Rubiks::render_rubik(glm::mat4 view, glm::mat4 proj, double frame_time)
{
    /*        Motion    - start        */
...
void Rubiks::draw_back_buffer(glm::mat4 view, glm::mat4 proj)
{
    glm::mat4 model = rubiks_rot * rubiks_trans * rubiks_scl;
    glm::mat4 mvp = proj * view * model;
